...
- **처음 보는 버그.** 위의 사전 등록 재현에서 부류 안 재현 7건 가운데 0건을 잡았다. entail이 아직 읽지 않는 사실: 접두 캐시 키의 구성, 커널이 가정하는 입력 strides, 회전 임베딩의 짝짓기 방식, 라우팅 가중치가 속한 행. 7건 가운데 5건은 어댑터가 없는 자리(맨 커널 호출, 요청 파서, 어댑터 설정 파일, 접두 캐시 키, 빔 재정렬)에서 생겼다. 그 뒤 어댑터 설정 파일과 추론 파서의 설정 이름은 읽게 됐고(위의 두 행), 나머지는 아직이다. 그 두 행은 7건 가운데 둘을 뒤에 고친 회고이지 검출이 아니다. 다음 검출률은 새 어휘를 동결한 2차 사전 등록 재현에서 나온다.
