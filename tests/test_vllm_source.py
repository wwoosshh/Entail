...
from entail.adapters.vllm_source import Checkpoint, check_layer  # noqa: E402
...
def test_fused_qkv_matches():
    ...
    out, why = check_layer(PREFIX + "self_attn.qkv_proj", fused, CFG, ckpt)
    assert out == [] and why.startswith("compared"), (out, why)
...
def test_fused_qkv_with_a_rolled_source():
    ...
    out, _ = check_layer(PREFIX + "self_attn.qkv_proj", fused, CFG, ckpt)
    assert len(out) == 1 and "q_proj.weight" in out[0], out
...
def test_swapped_k_and_v_is_caught():
    ...
    out, _ = check_layer(PREFIX + "self_attn.qkv_proj", fused, CFG, ckpt)
    assert len(out) == 1 and ("k_proj" in out[0] or "v_proj" in out[0]), out
...
def test_gate_up_fusion():
    ...
    ok, why = check_layer(PREFIX + "mlp.gate_up_proj", torch.cat([gate, up], 0), CFG, ckpt)
    assert ok == [] and why.startswith("compared"), (ok, why)
    bad, _ = check_layer(PREFIX + "mlp.gate_up_proj", torch.cat([up, gate], 0), CFG, ckpt)
...
def test_row_count_mismatch_is_reported():
    ...
    ckpt = make_checkpoint(**{PREFIX + "self_attn.q_proj.weight": q, PREFIX + "self_attn.k_proj.weight": k,
                              PREFIX + "self_attn.v_proj.weight": v})
    out, _ = check_layer(PREFIX + "self_attn.qkv_proj", torch.cat([q, k], 0), CFG, ckpt)
    assert len(out) == 1 and "add up to" in out[0], out


def test_unknown_layer_is_skipped_not_passed():
    ckpt = make_checkpoint(**{PREFIX + "self_attn.q_proj.weight": torch.zeros(8, 8)})
    out, why = check_layer(PREFIX + "self_attn.some_new_proj", torch.zeros(8, 8), CFG, ckpt)
    assert out == [] and why == "no declared source mapping", (out, why)
